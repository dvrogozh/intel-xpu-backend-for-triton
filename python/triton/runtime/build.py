...
def _check_cxx_option(cxx: str, option: str) -> bool:
    """
    Checks if compiler supports a given command-line option.

    :param cxx: Name or path of the compiler (e.g., 'gcc', 'clang', 'g++').
...
@functools.lru_cache()
def _is_compatible_sycl_compiler(cxx: str) -> bool:
    filename = os.path.basename(cxx)
    if filename != "icpx" and not filename.startswith("dpclang"):
        if not _check_cxx_option(cxx, "-fsycl"):
            return False
    return filename == knobs.intel.sycl_compiler if knobs.intel.sycl_compiler else True
...
def _build(name: str, src: str, srcdir: str, library_dirs: list[str], include_dirs: list[str], libraries: list[str],
    ...
    if is_xpu():
        ...
        if _is_compatible_sycl_compiler(cc):
