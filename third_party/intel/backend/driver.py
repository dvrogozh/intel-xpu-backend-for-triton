...
import triton
from triton.runtime.build import _build, platform_key, _load_module_from_path, _find_xpu_compiler, _is_compatible_sycl_compiler
...
def find_sycl_dpclang(include_dir: list[str]) -> tuple[list[str], list[str]]:
    ...
    dpclangpp = shutil.which(knobs.intel.sycl_compiler if knobs.intel.sycl_compiler else "dpclang++")
    cmd = [dpclangpp, "-E", "-dM", "-"]
...
def find_sycl(include_dir: list[str]) -> tuple[list[str], list[str]]:
    ...
    # SYCL capable compiler.
    if _is_compatible_sycl_compiler(_find_xpu_compiler()):
        return include_dir, []
